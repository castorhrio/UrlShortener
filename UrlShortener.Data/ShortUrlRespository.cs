...
public sealed class ShortUrlRespository
{
    private readonly ConnectionMultiplexer redisConnection;
    private readonly IDatabase redisDatabase;

    public ShortUrlRespository(ConnectionMultiplexer redisConnection)
    {
    ...
    public async Task Create(ShortUrl shortUrl)
    {
        ...
        if (!urlWasSet)
            throw new Exception($"Failed to create shortend URL");
    }

    public async Task Update(ShortUrl shortUrl)
    {
        if (await Exists(shortUrl.Path) == false)
            throw new Exception($"Shortened URL with path '{shortUrl.Path}' dose not exist");
    ...
    public async Task Delete(string path)
    {
        if (await Exists(path) == false)
            throw new Exception($"Shortened URL with pat '{path}' does not exist.");
    ...
    public async Task<bool> Exists(string? path) => await redisDatabase.KeyExistsAsync(path);
}